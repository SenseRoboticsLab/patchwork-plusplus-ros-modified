...
public:
  KittiLoader(const std::string &abs_path) {
    ...
    for (num_frames_ = 0;; num_frames_++) {
      std::string filename =
          (boost::format("%s/%06d.bin") % pc_path_ % num_frames_).str();
    ...
    for (num_labels = 0;; num_labels++) {
      std::string filename =
          (boost::format("%s/%06d.label") % label_path_ % num_labels).str();
      if (!boost::filesystem::exists(filename)) {
        break;
      }
    }
    ...
    if (num_frames_ == 0) {
      std::cerr << "\033[1;31mError: No files in " << pc_path_ << "\033[0m"
                << std::endl;
    }
  ...
  //        std::string filename = (boost::format("%s/%06d.bin") % pc_path_ %
  //        idx).str(); FILE *file = fopen(filename.c_str(), "rb"); if (!file) {
  //            std::cerr << "error: failed to load " << filename << std::endl;
  //            return -1;
  //        }
  ...
  //        size_t num_points = fread(reinterpret_cast<char *>(buffer.data()),
  //        sizeof(float), buffer.size(), file) / 4; fclose(file);
  ...
  //            std::string label_name = (boost::format("%s/%06d.label") %
  //            label_path_ % idx).str(); std::cout << label_name << std::endl;
  ...
  //            label_input.read((char*)&labels[0], num_points *
  //            sizeof(uint32_t));
  //
  //            for (int i = 0; i < num_points; i++) {
  //                auto &pt = cloud.at(i);
