...
void callbackCloud(const sensor_msgs::PointCloud2::Ptr &cloud_msg) {
    ...
    pcl::PointCloud<hesai::PointXYZIT> cloud_in_hesai;
    pcl::PointCloud<PointType> cloud_in;
    // pcl::PointCloud<PointType> pc_curr;
    pcl::PointCloud<PointType> pc_ground;
    pcl::PointCloud<PointType> pc_non_ground;

    pcl::fromROSMsg(*cloud_msg, cloud_in_hesai);

    for (const auto &point : cloud_in_hesai.points) {
        PointType pt;
        pt.x = point.x;
        pt.y = point.y;
        pt.z = point.z;
        pt.intensity = point.intensity;
        pt.t = point.timestamp * 1e9;
        pt.label = frame_idx;
